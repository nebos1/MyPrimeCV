...
function PhotoImportField({ onChange }) {
    ...
    return (
        <div className="create-cv-photo-import">
            <button type="button" className="create-cv-photo-import-btn" onClick={() => inputRef.current?.click()}>
                <Icons.ImagePlusLucideIcon />
                Import photo
            </button>
            <input ref={inputRef} type="file" accept="image/*" onChange={HandleFileChange} hidden />
            {error && <span className="create-cv-photo-import-error">{error}</span>}
        </div>
    );
}
...
function BasicsEditor({ basics, onChange }) {
    return (
        <SidebarSection icon={Icons.UserRoundLucideIcon} title="Personal details">
            <PhotoImportField onChange={(value) => onChange("photo", value)} />
